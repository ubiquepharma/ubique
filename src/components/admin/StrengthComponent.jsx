...
const StrengthComponent = ({ heading }) => {
  ...
  const handleEdit = async (id, strength) => {
    try {
      await axios.patch(`/api/strength`, { id, strength });
      seteditingId(null);
      fetchStrengths();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete("/api/strength", { data: { id } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post("/api/strength", formData);

      if (!res.data.success) {
        alert("Failed to add strength. Strength must be unique");
      }
      setLoading(false);
      setFormData("");
      fetchStrengths();
                  ...
                  {editingId === strength.id ? (
                    <div className="w-[80%] flex items-center gap-2">
                      <input
                        ...
                        defaultValue={strength.strength}
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleEdit(strength.id, formData.strength)
                        }
                      >
                        Submit
                      </button>
