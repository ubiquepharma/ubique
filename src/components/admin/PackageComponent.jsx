...
const PackageComponent = ({ heading }) => {
  ...
  const handleEdit = async (id, packaging) => {
    try {
      await axios.patch(`/api/packaging`, { id, packaging });
      seteditingId(null);
      fetchPackagings();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete("/api/packaging", { data: { id } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post("/api/packaging", formData);

      if (!res.data.success) {
        alert("Failed to add packaging. Packaging must be unique");
      }
      setLoading(false);
      setFormData("");
      fetchPackagings();
                  ...
                  {editingId === packaging.id ? (
                    <div className="w-[80%] flex items-center gap-2">
                      <input
                        ...
                        defaultValue={packaging.packaging}
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleEdit(packaging.id, formData.packaging)
                        }
                      >
                        Submit
                      </button>
